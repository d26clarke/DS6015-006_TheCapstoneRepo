...
def load_tile_list(csv_path: str,
    ...
    current_only  : If True, skip rows where VComment contains "Replaced" (superseded tiles)
    """
    ...
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            # Optional: skip superseded tiles
            if current_only:
    ...
    if skipped_superseded:
        logger.info(f"  Skipped {skipped_superseded} superseded tiles (--current-only)")
    if skipped_no_url:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Skip superseded tiles (VComment contains 'Replaced')"
    )
    parser.add_argument(
