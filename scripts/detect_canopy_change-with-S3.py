...
def process_tile_pair(path1: Path, path2: Path, out_dir: Path, 
                      year1: str, year2: str, overwrite: bool,
                      s3_bucket: str = None, county: str = "unknown") -> dict:
    ...
    try:
        ...
        if s3_bucket:
            ...
            if s3_client:
                year_tag = f"{year1}_{year2}"
                s3_key = s3_utils.build_s3_key("change", county, year_tag, out_path.name)
                s3_utils.upload_file(s3_client, out_path, s3_bucket, s3_key)
...
def main():
    ...
    parser.add_argument("--s3-upload", action="store_true", help="Upload outputs to AWS S3 bucket (central-virginia-tree-canopy-project)")
    ...
    for idx, (p1, p2) in enumerate(matched_pairs, 1):
        ...
        stats = process_tile_pair(p1, p2, args.out, args.year1, args.year2, args.overwrite, s3_bucket, args.county)
        if stats:
    ...
    # S3 Upload for the master statistics CSV
    if s3_bucket:
        s3_client = s3_utils.get_s3_client()
        if s3_client:
            logger.info("Uploading change statistics to S3...")
            year_tag = f"{args.year1}_{args.year2}"
            s3_key = s3_utils.build_s3_key("change", args.county, year_tag, csv_path.name)
            s3_utils.upload_file(s3_client, csv_path, s3_bucket, s3_key)
