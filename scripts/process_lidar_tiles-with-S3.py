...
# ── Configuration Constants ─(all metric)───────────────────────────────────────
FT_TO_M              = 0.3048006096   # 1 US survey foot in metres (exact)
OUTPUT_CRS           = "EPSG:6591"    # NAD83(2011) / Virginia State Plane North, metres
...
SKIP_LOG_PATH        = Path("/home/thq3hn/development/DS6015-006/data/outputs/Charlottesville/skipped_tiles.csv")
UNCLASS_LOG_PATH     = Path("/home/thq3hn/development/DS6015-006/logs/unclassified_tiles.log")
COMBINED_CENTROID    = Path("")
RUN_SUMMARY_PATH     = Path("/home/thq3hn/development/DS6015-006/data/outputs/Charlottesville/run_summary.txt")
...
def process_file(url: str, geotiff_filename: str,
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            # ── Stage 3b: Unit Conversion (US survey feet → metres) ───────────
            t0 = time.perf_counter()
            x = x * FT_TO_M
            y = y * FT_TO_M
            z = z * FT_TO_M
            ...
            logger.info(
                f"[{pid}] Unit conversion ft→m: {t_convert:.2f}s  "
                f"(x_range: {float(x.min()):.1f}–{float(x.max()):.1f} m, "
                f"z_range: {float(z.min()):.1f}–{float(z.max()):.1f} m)"
            )
            ...
            with rasterio.open(
                ...
                dtype=chm.dtype, crs=OUTPUT_CRS,
...
if __name__ == "__main__":
    ...
    GEOTIFF_OUTPUT_DIR = Path(f"/home/thq3hn/development/DS6015-006/data/outputs/Charlottesville/GeoTIFF_files/{county_tag}_{year_tag}/")
    CENTROID_OUTPUT_DIR = Path(f"/home/thq3hn/development/DS6015-006/data/outputs/Charlottesville/centroids/{county_tag}_{year_tag}/")
    COMBINED_CENTROID = Path(f"/home/thq3hn/development/DS6015-006/data/outputs/Charlottesville/{county_tag}_{year_tag}_centroids.csv")
