...
-----
    ...
    # Skip years whose output files already exist (resume after timeout)
    python smap_annual_mean_from_s3.py --years 2021 2022 2023 --skip-existing
...
def read_sm_from_s3(fs: s3fs.S3FileSystem, s3_path: str,
    ...
    with h5py.File(io.BytesIO(raw), "r") as hf:
        for grp_name in groups:
            ...
            # ISSUE: We are only getting ~9 pizels per year; this is too restrictive
            # Time to relax the Qua;ity Flag less than or equal to 1
            # ((qf & QUALITY_MASK) == 0) &
            # This is the standard approach used in peer-reviewed SMAP studies
            # over forested terrain (O'Neill et al.  2021, NSIDC SPL3SMP_E ATBD). Expected result after the fix:
            valid = (
                (sm != FILL_VALUE) &
                ((qf & QUALITY_MASK) <= 1) &
                bbox_mask
...
def compute_annual_means(
    ...
    for year in sorted(by_year.keys()):
        ...
        # ── Skip-existing check ───────────────────────────────────────────
        if skip_existing:
            tif_path_check = out_path / f"smap_annual_mean_{year}.tif"
            csv_path_check = out_path / f"smap_annual_mean_{year}.csv"
            if tif_path_check.exists() and csv_path_check.exists():
                print(f"\n  Skipping {year} — output files already exist:")
                print(f"    {tif_path_check}")
                print(f"    {csv_path_check}")
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "Skip any year whose GeoTIFF and CSV output files already exist "
            "in --out-dir. Use this to resume a job that was interrupted or "
            "timed out without reprocessing completed years."
        )
    )
    return p.parse_args()
